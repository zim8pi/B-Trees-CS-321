...
public class ParsingFile {
	...
	public NodeObject[] parseDNAFile(String fileName, int sequenceLength) {
		try {
			subSequence = new StringBuilder(sequenceLength);
			...
			while ((line = buffer.readLine()) != null)
			{
				...
				//TODO will the '//' always be on a separate line than the ATGC's????
				if (line.contains("//"))
				{
					break;
				}
				...
				while (character[i] != null) 
				{
					...
					if (character[i].equals("A") || character[i].equals("a") || character[i].equals("T") || character[i].equals("t") ||
							character[i].equals("C") || character[i].equals("c") || character[i].equals("G") || character[i].equals("g") ||
